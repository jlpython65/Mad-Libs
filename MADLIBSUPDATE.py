...
blanks_list = re.findall(r'___\(.+?\)___',story)
story_list = story.split()


story_index = [i for i, s in enumerate(story_list, start=0) if re.search(r'___\(.+?\)___', s)] 
for x in range(0,len(blanks_list)):
    prompt(blanks_list[x],story_list,story_index[x])
